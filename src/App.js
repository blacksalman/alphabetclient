...
function App() {
  ...
  const [operatorArr] = useState(["+","-","*","/"]);
  const [comparatorArr] = useState(["<",">"]);
  ...
  useEffect(()=>{
    axios.get("https://alphabetapiserver.herokuapp.com/alphabets")
  ...
  function removeElement(e){
    ...
    setEquationArr(equationArr.filter((elem, index)=>{
  ...
  function handleMouseUp(e){
    if(tempDragGraphic){
      ...
      let isInCanvasElement = e.clientY+window.scrollY>canvasElementTop 
  ...
  return (
    <div className="App" onMouseMove={(e)=>handleMouseMove(e)} onMouseUp={(e)=>handleMouseUp(e)}>
      <div className="tempDragGraphic" 
      style={{position:'fixed', opacity: 0.6, zIndex: 3, left:mouseCoords.x, top:mouseCoords.y}}>
      {
      ...
      <div className="canvas"  ref={canvasElement}>
        {
          ...
          <div className={elem.type} key={index}>
            <span key={index} className="remove" onClick={(e)=>removeElement(e)}
             data-value={index}> x </span>{elem.alphabet}</div>)
        }

        {
          comparator && <div className="comparator">
            <span className="remove" onClick={()=>setComparator("")}>x</span>{comparator}</div>
        }

        {
          rhs && <div className="rhs">
            <span className="remove" onClick={()=>setRhs("")}>x</span>{rhs}</div>
        }
      </div>
